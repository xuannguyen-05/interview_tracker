...
export default function Funnel({ funnel }) {
  ...
  const chartData =
    funnel?.map((item) => ({
      ...
      fill:
        item.label === t("application.applied")
          ? "#3B82F6"
          : item.label === t("application.interview")
            ? "#8B5CF6"
            : "#10B981",
    })) || [];
      ...
      return (
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-lg">
          ...
          <p className="text-sm text-slate-600">
            {data.value} {t("dashboard.funnelTooltip.applications")}
          </p>
          {data.name !== t("application.applied") &&
            data.percent !== undefined && (
              <p className="mt-1 text-xs text-slate-400">
                {data.percent.toFixed(1)}%{" "}
                {t("dashboard.funnelTooltip.conversion")}
              </p>
            )}
        </div>
      );
